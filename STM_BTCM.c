            ...
            if (BTType == 1) // RX mode
            {
                ...
                if (FqRXCount == 0)
                    if (DataB0.RXLoopAllowed) // if it was request to TX then need to switch off round-robin
                    {
                        ...
                        DataB0.RXLoopBlocked = 1; // flag successfull blocking set 
...
SEND_GOOD:      BTbyteCRC(BTqueueOutCopy[i]);
      ...
      bitset(PORT_BT,Tx_CSN);
        ...
        if (FqTXCount ==0) // over FQ1 need to switch to TX mode == for FQ2 & FQ3 it is already done
        {
            bitclr(PORT_BT,Tx_CSN);
        ...
        {
SWITCH_ANOTHER:
            SwitchFQ(FqTX); // in that call for TX over FQ1 value of TXSendOverFQ == 1
